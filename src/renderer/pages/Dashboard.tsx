...
export default function Dashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      load(7, setRemoteAccess, 'remote access');

      if (!failed.length) {
        setMessage('');
      } else if (failed.includes('health') || failed.includes('status')) {
        setMessage(`Talocode is offline at ${API}. Start it with: talo start.`);
      } else {
        setMessage(`Some sections are still loading: ${failed.join(', ')}.`);
      }
    } catch {
      setMessage(`Talocode is offline at ${API}. Start it with: talo start.`);
    } finally {
...
async function post<T,>(path: string, body: unknown): Promise<T> { return request<T>(path, { method: 'POST', body: JSON.stringify(body) }); }
async function del<T,>(path: string): Promise<T> { return request<T>(path, { method: 'DELETE' }); }
