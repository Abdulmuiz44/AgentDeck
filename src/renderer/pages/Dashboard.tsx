...
const API = import.meta.env.VITE_AGENTDECK_API_URL || 'http://127.0.0.1:3768';
...
export default function Dashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setMessage(`Daemon is offline. Start it with: agentdeck start (${API})`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="dashboard-shell">
      <aside className="cockpit-sidebar">
        <div className="brand-block"><div className="brand-mark">AD</div><div><h1>AgentDeck</h1><span>Agent cockpit</span></div></div>
...
function Home({ health, status, providers, detected, projects, sessions, remoteAccess, onNavigate, onRemoteChanged }: { health: DaemonHealth | null; status: DaemonStatusView | null; providers: ProviderConfigView[]; detected: DiscoveryView[]; projects: ProjectView[]; sessions: SessionView[]; remoteAccess: RemoteAccessView | null; onNavigate: (page: Page) => void; onRemoteChanged: () => Promise<void> }) {
  return <div className="grid-page">
    <Stat title="Daemon" value={health ? 'Online' : 'Offline'} detail={health ? `${health.platform} · ${health.uptime}s uptime` : 'Run agentdeck start'} />
...
function Architecture() {
  const sections = [
    { title: 'Access Layer', items: ['Phone Control', 'Desktop Dashboard', 'CLI', 'Local API Clients'], detail: 'Every surface talks to the same local AgentDeck daemon.' },
    { title: 'AgentDeck Core', items: ['Daemon / Background Service', 'Local HTTP API', 'Session Manager', 'Project & Config Manager', 'Discovery Engine'], detail: 'Core orchestration owns state, lifecycle, discovery, status, and safe config generation.' },
    ...
    { title: 'Execution & Storage Layer', items: ['Local Project Paths', 'Child Processes / Agent Sessions', 'Logs & Session History', 'Settings / Local Persistence'], detail: 'Sessions run in validated project directories and persist logs/settings under the OS app-data directory.' },
    { title: 'Outcomes', items: ['Discover installed tools', 'Configure providers', 'Launch and resume sessions', 'Generate Codex/OpenCode configs', 'Manage projects across devices'], detail: 'AgentDeck turns local tools and providers into a cockpit that works from desktop, CLI, API, and phone.' },
  ];
  return <div className="architecture-page"><section className="panel hero-panel"><p className="eyebrow">Mermaid source of truth</p><h3>AgentDeck Architecture</h3><p className="muted">The architecture diagram is text-based in <code>docs/agentdeck-architecture.mmd</code> so PRs stay reviewable. The dashboard renders the same structure as React/CSS cards instead of binary image assets.</p></section><div className="architecture-map">{sections.map((section, index) => <section className="panel architecture-card" key={section.title}><div className="architecture-step">{index + 1}</div><h3>{section.title}</h3><p className="muted">{section.detail}</p><div className="architecture-items">{section.items.map((item) => <span key={item}>{item}</span>)}</div></section>)}</div></div>;
}
