...
const TOKEN_KEY = 'agentdeck.phone.accessToken';
const DEVICE_KEY = 'agentdeck.phone.device';
...
export default function PhoneDashboard() {
  ...
  if (!accessToken) {
    return <main className="phone-shell"><section className="phone-card hero"><div className="brand-mark">AD</div><h1>Pair AgentDeck</h1><p>Pair this phone only when it is on the same trusted LAN as your desktop. The token is shown on the desktop only when generated or rotated.</p><label>Pairing token<input value={pairToken} onChange={(event) => setPairToken(event.target.value)} placeholder="Paste token from desktop" autoComplete="one-time-code" /></label><button className="primary touch" disabled={!pairToken.trim() || busy} onClick={() => void pair()}>{busy ? 'Pairing…' : 'Pair this phone'}</button>{message && <p className="warning-text">{message}</p>}</section></main>;
  }

  const sessions = snapshot?.sessions || [];
  const activeSessions = sessions.filter((session) => session.status === 'running');

  return <div className="phone-shell paired"><header className="phone-header"><div><p className="eyebrow">AgentDeck phone control</p><h1>{label(tab)}</h1></div><StatusBadge status={snapshot ? 'online' : 'offline'} /></header>{message && <div className="notice warning">{message}</div>}
    {tab === 'home' && <section className="phone-grid"><PhoneStat title="Daemon" value={snapshot?.status?.status || 'offline'} /><PhoneStat title="Active" value={String(snapshot?.status?.activeSessions || activeSessions.length)} /><PhoneStat title="Projects" value={String(snapshot?.projects.length || 0)} /><PhoneStat title="Providers" value={String(snapshot?.providers.length || 0)} /><div className="phone-card"><h2>Quick actions</h2><button className="touch" onClick={() => setTab('sessions')}>Manage sessions</button><button className="touch" disabled={busy} onClick={() => void refresh()}>Refresh cockpit</button></div><SessionList sessions={sessions.slice(0, 4)} onOpen={openSession} onAction={sessionAction} /></section>}
    ...
    {tab === 'agents' && <section className="phone-grid">{snapshot?.agents.length ? snapshot.agents.map((agent) => <div className="phone-card" key={agent.id}><div className="panel-title"><h2>{agent.displayName}</h2><StatusBadge status={agent.status} /></div><p>{agent.executableNames.join(', ')}</p><small>{agent.installHint || 'Install this tool on the desktop to use it.'}</small></div>) : <EmptyCard text="No agent adapters were returned by the daemon." />}</section>}
    {tab === 'settings' && <section className="phone-card"><h2>Phone settings</h2><p>Device: {device?.name || 'paired phone'}</p><p>Connection: {window.location.origin}</p><p className="warning-text">LAN-only MVP. Disable or revoke access from the desktop when finished.</p><button className="touch" onClick={() => void logout()}>Logout this phone</button></section>}
...
function safeStoredDevice(): PhoneDevice | null { try { return JSON.parse(window.localStorage.getItem(DEVICE_KEY) || 'null') as PhoneDevice | null; } catch { window.localStorage.removeItem(DEVICE_KEY); return null; } }
