...
Phone Control lets Talocode act as an always-on cockpit for sessions running on your desktop. The MVP is local-network only: your phone browser connects directly to the Talocode daemon on the same trusted Wi-Fi/Ethernet network.

Implemented behavior:

- Phone access is disabled by default.
- Desktop/CLI must explicitly enable access.
- Talocode detects non-loopback LAN IPv4 candidates and can bind to `0.0.0.0` when enabled.
...
1. Start Talocode on the desktop.
2. Open the dashboard and go to **Settings → Phone Control**.
3. Click **Enable phone access**.
4. Talocode generates a one-time pairing token and a phone URL such as `http://192.168.1.20:3768/phone?pair=...`.
...
Open `/phone` on the LAN URL. If the URL includes `?pair=TOKEN`, tap **Pair this phone**. Otherwise paste the token from the desktop dashboard or `talo remote enable` output.
...
- **Firewall blocks port:** allow TCP port `3768` for the Talocode process in Windows Firewall.
- **Wrong Wi-Fi:** guest networks often block peer-to-peer LAN traffic.
- **Daemon bound to localhost:** enabling phone control stores `bindHost=0.0.0.0`; restart Talocode if the listener cannot rebind automatically.
- **Token expired/revoked:** rotate the token on desktop and pair again.

## Partial/fallback behavior

- Native QR image rendering is not implemented yet; Talocode displays a QR payload/phone URL.
